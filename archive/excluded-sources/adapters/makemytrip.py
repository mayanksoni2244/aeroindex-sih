"""
Tier-1.5 live adapter: MakeMyTrip (MMT), via Playwright.

ToS Citation (https://www.makemytrip.com/legal/in/eng/user_agreement.html):
"User shall not distribute exchange, modify, sell or transmit anything from the Website, including but not limited to any text, images, audio and video, for any business, commercial or public purpose. The User Agreement grants a limited, non-exclusive, non-transferable right to use this Website as expressly permitted in this User Agreement."
...
logger = logging.getLogger("apix.scrape.live.makemytrip")

class MakeMyTripSource(LiveSource):
    name = "MakeMyTrip (limited-scope)"
    base_url = "https://www.makemytrip.com"
    default_source_type = SourceType.LIVE_LIMITED

    URL_TEMPLATE = (
        "{base}/flight/search"
        "?itinerary={origin}-{destination}-{travel_date}"
    ...
    def search_url(self, origin: str, destination: str, travel_date: date) -> str:
        # MakeMyTrip expects dd/mm/yyyy for its URL? Wait, let's look at the OTA probe output.
        # Typically mm/dd/yyyy or dd/mm/yyyy. Let's use dd/mm/yyyy. We'll adjust if it complains.
        # Actually it's often dd/mm/yyyy. Let's assume dd/mm/yyyy.
        # Wait, the URL template has itinerary=DEL-BOM-11/10/2026? Itinerary format usually DEL-BOM-DD/MM/YYYY.
        # Let's use DD/MM/YYYY. If it's different, it'll just fail to parse fares.
